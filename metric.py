...
class GenericEvaluator:
    def __init__(self, result_path):
        self.result_file = open(result_path, 'r')
        self.meta = []
        self.metric_dict = {}
        self.entity_dict = {}
    ...
    def read_result_data(self):
        ...
        reader = csv.DictReader(self.result_file)
    ...
    def clean_multiwoz(self, s):
        s = s.replace('<go> ', '')
        s = '<GO> ' + s + ' </s>'
        for item in self.entity_dict:
            entity = " ".join(item.split(" ")[1:])
            # s = s.replace(item, 'VALUE_{}'.format(self.entity_dict[item]))
            tp = " ".join(self.entity_dict[item].split(" ")[1:])
            slot = "[value_" + tp + "]"
            s = clean_replace(s, entity, slot)
        return s
...
class CamRestEvaluator(GenericEvaluator):
    ...
    def run_metrics(self):
        raw_json = open('./data/CamRest676/CamRestFormal.json')
        ...
        for i, row in enumerate(data):
            data[i]['response'] = self.clean_camrest(data[i]['response'])
            data[i]['generated_response'] = self.clean_camrest(data[i]['generated_response'])
...
class MultiWOZEvaluator(GenericEvaluator):
    ...
    def run_metrics(self):
        raw_json = open('./data/MultiWOZ/data_formal.json')
        raw_entities = open('./data/MultiWOZ/value_set_processed.json')
        raw_data = json.loads(raw_json.read().lower())
        raw_entities = json.loads(raw_entities.read().lower())
        self.get_entities(raw_entities)
        data = self.read_result_data()
        for i, row in enumerate(data):
            data[i]['response'] = self.clean_multiwoz(data[i]['response'])
            data[i]['generated_response'] = self.clean_multiwoz(data[i]['generated_response'])
        bleu_score = self.bleu_metric(data, 'bleu')
        success_f1 = self.success_f1_metric(data, 'success')
        match = self.match_metric(data, 'match', raw_data=raw_data)
        self._print_dict(self.metric_dict)
    ...
    def get_entities(self, entity_data):
        for k in entity_data:
            domain = "[" + k + "] "
            for key, value in entity_data[k].items():
                dom_key = domain + key
                dom_value = []
                for v in value:
                    dom_value.append(domain + v)
                    self.entity_dict[domain + v] = dom_key
                self.entities.extend(dom_value)

    def _extract_constraint_multi(self, z):
        z = z.split('<cat>')
        domain = None
        dom_z = []
        for word in z:
    ...
    def match_metric(self, data, sub='match', raw_data=None):
        ...
        for dial_id in dials:
            ...
            gen_bspan = dial[-1]['generated_bspan']
            gen_cons = self._extract_constraint_multi(gen_bspan)

            truth_cons = self._extract_constraint_multi(dial[-1]['bspan'])

            for token in gen_cons:
                if token in truth_cons:
                    tp += 1
                else:
                    fp += 1
            for token in truth_cons:
                if token not in gen_cons:
                    fn += 1
